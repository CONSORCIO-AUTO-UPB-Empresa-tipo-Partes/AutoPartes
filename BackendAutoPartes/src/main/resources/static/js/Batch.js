...
formAgregarLote.addEventListener("submit", function (e) {
    ...
    if (editando) {
        ...
        const index = lotes.findIndex(l => l.idBatch === loteEditandoId);
        if (index !== -1) {
            lotes[index] = lote;
        }
        editando = false;
...
function editarLote(idBatch) {
    const lote = lotes.find((l) => l.idBatch == idBatch); // Usar == en lugar de === para comparación flexible
    if (lote) {
        ...
        editando = true;
        loteEditandoId = idBatch;
    }
}

function eliminarLote(idBatch) {
    if (confirm("¿Estás seguro de que deseas eliminar este lote?")) {
        lotes = lotes.filter((l) => l.idBatch != idBatch); // Usar != en lugar de !==
        actualizarTablaLotes();

        // Si estamos editando el lote que se eliminó, resetear el estado
        if (editando && loteEditandoId == idBatch) {
            editando = false;
            loteEditandoId = null;
            formAgregarLote.reset();
        }
